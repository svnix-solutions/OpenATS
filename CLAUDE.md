...
- **File reads** (`/files/*`) are mounted outside `/api` because the auth boundary differs per folder rather than per router: `logos/` is anonymous (careers pages render them for visitors with no account), `resumes/` runs `authMiddleware` and then `canReadResumeKey`. Neither serves bytes — both answer with a 302 to a short-lived signed URL, so the bucket stays private and byte-range requests stay the bucket's problem. The browser reaches them through `frontend/app/api/files/[...key]/route.ts`, which exists because an `<iframe src>` carries no `Authorization` header but does carry the session cookie. `R2_PUBLIC_URL` is what points stored URLs at this instead of at the bucket; see `docs-draft/STORAGE.md`.
...
- **Redis + BullMQ**: CV analysis and bulk candidate import run as background job queues, both consumed by `worker.ts`. A candidate import is a `candidate_imports` row carrying the CSV and its progress — the worker advances `processed` as it goes and clears the file when it finishes, because it is a list of people's contact details. CV analysis runs as a background job queue, colocated under `backend/src/queues/cv-analysis/` (`queue.ts`, `worker.ts`, `events.ts`); shared Redis connection factory is `backend/src/config/redis.ts`. Connection is read from `REDIS_URL` (defaults to `redis://localhost:6379`). A dedicated connection is created per Queue/Worker (BullMQ best practice), not a shared singleton.
...
- **One deployment path: Komodo.** `komodo/` deploys the whole application — database, identity provider, API, queue worker, frontend — as a [Komodo](https://komo.do) Stack; see `docs-draft/DEPLOY_KOMODO.md`. `.github/workflows/images.yml` builds the three images and pushes them to GHCR on every push to `main`, and the server pulls them rather than building: it has no reason to hold a TypeScript compiler and a dependency tree for something it runs once, and an image built there is produced by whatever that box has installed rather than by what CI tested.
- There was a second path, an SSH deploy of the backend alone to an Azure VM under pm2. It is gone. It had never run successfully from this fork — `SSH_HOST` and friends are repository secrets and GitHub does not give those to forks — and it depended on an `ecosystem.config.js` that was never in the repository, so nobody could reproduce it. `test.yml` no longer tars and uploads `backend/dist`; that artifact existed only to feed it.
