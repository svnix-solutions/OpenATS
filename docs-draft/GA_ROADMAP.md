...
| Candidate messaging over WhatsApp and Telegram | Schema, the channel contract and the encrypted org-level credential store are in. WhatsApp initiates with an approved template and is free-form only inside the 24h window a candidate's reply opens; Telegram is MTProto, which is a held connection rather than a webhook and so belongs in the worker. Providers, the webhook route and the candidate inbox still to come | 🟡 In progress |
