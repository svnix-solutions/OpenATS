...
- Authenticated E2E tests are not set up. Logging in through the identity provider in a test means driving a real sign-in flow, which needs credentials and is slow and flaky. Prefer testing `/public/*` routes, which skip authentication entirely, or save a Playwright `storageState` from one manual login and reuse it.
