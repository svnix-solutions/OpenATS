...
- **Fixture queries are not scoped for you.** `e2e/seed.ts` connects as the database owner — a superuser — so row-level security does not filter it. An unqualified `SELECT id FROM departments LIMIT 1` picked up another spec's department, the job built on it held a foreign key into a world it did not own, and that world's teardown failed with a constraint violation in a different spec file. Key every fixture read to a row the world owns.
- **A failed teardown poisons later runs.** The organization left behind by that violation made nine tests fail on every subsequent run, for reasons unconnected to the change under test. `e2e/global-setup.ts` now deletes worlds left by an earlier run before anything starts.
- **`backend/.env.test` pins `SMTP_HOST=` empty on purpose.** `mail.service` calls `dotenv.config()` itself, so a developer who set `SMTP_HOST` in `backend/.env` — which is what the Mailpit instructions ask for — otherwise sends the suite down the SMTP path, and every test asserting what Resend was handed fails on their machine and nowhere else.
- **The E2E suite needs the mail catcher.** One spec sends a real email and asserts the server-rendered history, so `docker compose up -d mailpit` and `SMTP_HOST=localhost` in `backend/.env` are required — CI sets both. Without them the send fails, nothing is recorded (correctly: a history entry is a claim that someone was contacted), and the spec looks like a broken feature rather than a missing container. `e2e/global-setup.ts` says so on the way in.
- **Outgoing mail can be read, not just assumed.** `docker compose up -d mailpit` starts a catcher; set `SMTP_HOST=localhost` in `backend/.env` and every offer, rejection and interview email lands at http://localhost:8025 instead of being sent. The transport is the only thing that branches — branding, escaping and template substitution happen above it, and `tests/integration/mail-transport.test.ts` pins that both paths are handed the same From, Subject and body.
...
- **Never assert a redirect by its negative.** `await expect(page).not.toHaveURL(/x/)` after navigating to `/x` is satisfied by the instant before the redirect fires, so it passes whether the guard works or not — inverting it to `toHaveURL(/x/)` passed too, which is how it was caught. Assert where the guard *lands* you instead.
